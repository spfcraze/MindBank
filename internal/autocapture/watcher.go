...
// Watcher watches for Hermes session files and creates namespace-aware nodes.
type Watcher struct {
...
// ProcessFile reads a session file, derives namespace, and creates a node.
func (w *Watcher) ProcessFile(ctx context.Context, path string) error {
	...
	// Create node with derived namespace and metadata
