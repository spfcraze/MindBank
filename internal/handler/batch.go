...
// BulkDeleteNodes handles DELETE /api/v1/nodes/bulk — delete multiple nodes by ID.
func (h *BatchHandler) BulkDeleteNodes(w http.ResponseWriter, r *http.Request) {
	var req struct {
		IDs []string `json:"ids"`
	}
	if err := bindJSON(r, &req); err != nil {
	...
	if len(req.IDs) == 0 {
		respondError(w, 400, "ids array is required")
		return
	}
	if len(req.IDs) > 100 {
		respondError(w, 400, "max 100 nodes per bulk delete")
		return
	}
...
func RegisterBatchRoutes(r chi.Router, bh *BatchHandler) {
	...
	r.Delete("/nodes/bulk", bh.BulkDeleteNodes)
