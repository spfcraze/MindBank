...
func (h *AskHandler) Graph(w http.ResponseWriter, r *http.Request) {
	...
	if mi := r.URL.Query().Get("min_importance"); mi != "" {
		if v, err := strconv.ParseFloat(mi, 64); err == nil && v >= 0 && v <= 1 {
			minImportance = v
		}
	}
	...
	query := fmt.Sprintf(`SELECT n.id, n.namespace, n.label, n.node_type::text, n.content, n.summary, n.topic, n.importance, n.access_count, n.metadata, n.epistemic_label,
		...
		WHERE n.valid_to IS NULL AND n.node_type != 'session' AND n.importance >= $%d`, 1)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&n.ID, &n.NS, &n.Label, &n.NodeType, &content, &n.Summary, &n.Topic, &n.Importance, &n.AccessCount, &metadata, &n.EpistemicLabel, &n.EdgeCount); err != nil {
			continue
		}
		n.Metadata = metadata
